...
#  dplyr::mutate(Fahrzeug_Typ=as.factor(Fahrzeug_Typ))%>%
  
  dplyr::mutate(Werksnummer_OEM=str_sub(ID_Fahrzeug,6,7))%>%
#  dplyr::mutate(Werksnummer_OEM=as.factor(Werksnummer_OEM))%>%
  
  dplyr::mutate(Motor_Benennung=str_sub(ID_Motor,1,5))%>%
  dplyr::left_join(Abbildung_Typ_Benennung_Motor,by="Motor_Benennung")
#  dplyr::mutate(Motor_Typ=as.factor(Motor_Typ))


save(Datensatz_Final,file = "VariantData_str.RData")
