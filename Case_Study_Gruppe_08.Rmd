...
## Integration vorhandener Spalten
...
Um die Langlebigkeit von Dieselauto intuitiv zu überprüfen. Plotten wir die Überlebenswahrscheinlichkeit in Abhängigkeit von Lebensdauer(in Tag) nach `Kaplan-Meier` -Verfahren.
...
km <- survfit(Surv(Lebensdauer, Fehlerhaft) ~ Motor_Typ, data = Datensatz_Final)
...
Nach Graphik von  Überlebenswahrscheinlichkeit( bezogen auf aller Fahrzeug_Typ) ist die Dieselauto bei besitmmte Lebensdauer überall mit großer Wahrscheinlichkeit , längere  Zeit zu überleben.

In der App kannst du nur bestimmte Fahrzeug_Typ berücksichtigen.Und die Produktionsdatum ist auch  frei wählbar.
