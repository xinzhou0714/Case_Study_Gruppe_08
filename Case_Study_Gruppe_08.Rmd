...
Die Analyse der Casestudy fokusiert sich auf die Spezifikationen der zu erstellenden Shiny-Applikation. Die Spezifikationen beziehen sich auf drei Hauptanforderungen, die die Applikation erfüllen soll. Diese Anforderungen beschreiben die Graphiken und entsprechende Eigenschaften, die in der Applikation abgebildet werden sollen. 


Zuerst soll die Applikation ein Balkendiagramm, das die relativen Fehlerhäufigkeiten aller Motoren, getrennt in Benzin- und Dieselantrieb, darstellen. Das Diagramm muss interaktive Elemente enthalten und der Produktionszeitraum der Fahrzeuge muss durch den Nutzer variabel eingrenzbar sein. Basierend auf den Anforderungen wird zunächst das Verhältnis zwischen dem Anteil der fehlerhaften und defektlosen Motoren. Natürlich wird das separat je nach Antrieb durchgeführt. Dieses Verfahren kann dadurch einfach vervollständigen werden,      
...
# Inputs (erste Aufgabe der Visualisierung)
...
# Inputs (Zweite Aufgabe der Visualisierung)

      checkboxGroupInput("Fahrzeugtyp","Wählen Sie einen Fahrzeugtyp aus", choices = levels(Final_Datensatz$Fahrzeug_Typ), selected = "11"),
  
      checkboxGroupInput("Fahrzeugsmarke","Wählen Sie einen Fahrzeugsmarke aus", choices = levels(Final_Datensatz$Fahrzeugsmarke), selected = "OEM1")),
    
    mainPanel(
 
# Outputs (Erste Aufgabe der Visualisierung)

       tabsetPanel(
         
         tabPanel("Motorentyp", plotly::plotlyOutput("Balkendiagramm")),

# Outputs (Zweite Aufgabe der Visualisierung)

         tabPanel("Fahrzeugtyp", plotly::plotlyOutput("Boxplot_Fahreugtyp")),

         tabPanel("Fahrzeugsmarke", plotly::plotlyOutput("Boxplot_Fahreugsmarke")),

# Outputs (Dritte Aufgabe der Visualisierung)
...
# Angepasster Datensatz für den Balkendiagramm als Reactive Expression (Erste Aufgabe der Visualisierung)
...
# Angepasster Datensatz für den Boxplot der Fahrzeugtyp als Reactive Expression (Zweite Aufgabe der Visualisierung)
  
  Data_frame_Boxplot_Fahrzeugtyp <- reactive({
    Final_Datensatz %>%
      dplyr::filter(Produktionsdatum >= input$Produktionszeitraum[1], Produktionsdatum <= input$Produktionszeitraum[2]) %>%
      dplyr::filter( Fahrzeug_Typ %in% input$Fahrzeugtyp) %>%
      dplyr::filter(!is.na(Lebensdauer_in_Days)) %>%
      dplyr::select(c("Motor_Typ","Lebensdauer_in_Days")) 
  })
  
# Angepasster Datensatz für den Boxplot der Fahrzeugsmarke als Reactive Expression (Zweite Aufgabe der Visualisierung)
  
  Data_frame_Boxplot_Fahrzeugsmarke <- reactive({
    Final_Datensatz %>%
      filter(Produktionsdatum >= input$Produktionszeitraum[1], Produktionsdatum <= input$Produktionszeitraum[2]) %>%
      filter( Fahrzeugsmarke %in% input$Fahrzeugsmarke) %>%
      dplyr::filter(!is.na(Lebensdauer_in_Days)) %>%
      select(c("Motor_Typ","Lebensdauer_in_Days")) 
  })
  
  
# Erstellen der Reactive Endpoint (Erste Aufgabe der Visualisierung)  
  
  output$Balkendiagramm <- plotly::renderPlotly({
    ggplot(Data_frame_Balkendiagramm(), aes(x = Motor_Typ, y = Relative_Fehlerhaufigkeit)) + geom_bar(stat = "identity", fill = "DarkRed") + labs(x ="Motorentyp", y ="Fehlerhaufigkeit", title = "Relative Fehlerhäufigkeit der Benzin- und Dieselmotoren") + theme_minimal() + theme(text = element_text(family = "Bookman"),
    title = element_text(color = "gray25"), plot.background = element_rect(fill = "gray95"), plot.margin = unit(c(5, 10, 5, 10), units = "mm"))
  })
  
# Erstellen der Reactive Endpoint für den Fahrzeugtyp(Zweite Aufgabe der Visualisierung)  
  
  output$Boxplot_Fahreugtyp <- plotly::renderPlotly({
    ggplot(Data_frame_Boxplot_Fahrzeugtyp(), aes(x = Motor_Typ, y = Lebensdauer_in_Days)) + geom_boxplot(fill = "DarkRed") + labs(x ="Motorentyp", y ="Zeitintervall zwischen Zulassung und Fehlerdatum der Fahrzeuge (Tage)", title = "Lebensdauer der Fahrzeuge") + theme_minimal() + theme(text = element_text(family = "Bookman"), title = element_text(color = "gray25"), plot.background = element_rect(fill = "gray95"), plot.margin = unit(c(5, 10, 5, 10), units = "mm"))
  })
  
# Erstellen der Reactive Endpoint für die Fahrzeugsmarke (Zweite Aufgabe der Visualisierung)  
  
  output$Boxplot_Fahreugsmarke <- plotly::renderPlotly({
    ggplot(Data_frame_Boxplot_Fahrzeugsmarke(), aes(x = Motor_Typ, y = Lebensdauer_in_Days)) + geom_boxplot(fill = "DarkRed") + labs(x ="Motorentyp", y ="Zeitintervall zwischen Zulassung und Fehlerdatum der Fahrzeuge (Tage)", title = "Lebensdauer der Fahrzeuge") + theme_minimal() + theme(text = element_text(family = "Bookman"), title = element_text(color = "gray25"), plot.background = element_rect(fill = "gray95"), plot.margin = unit(c(5, 10, 5, 10), units = "mm"))
  })
...
ui <- fluidPage(
  ...
  titlePanel("Die Langlebigkeit von Dieselmotoren"),
 # titlePanel("Vergleich von Diesel- und Benzinmotoren"),
  
# Inputs (erste Aufgabe der Visualisierung)
  
img(src='https://www.berlin-brandenburg.de/wirtschaft-und-verkehr/qualitaetsinitiative/qw_gross.png', align = "left"),
...
plotly::plotlyOutput("Boxplot_Fahreugtyp"),

plotly::plotlyOutput("Boxplot_Fahreugsmarke")
...
# Angepasster Datensatz für den Balkendiagramm als Reactive Expression (Erste Aufgabe der Visualisierung)
  ...
  Data_frame_Boxplot_Fahrzeugsmarke <- reactive({
   ...
   output$Boxplot_Fahreugsmarke <- plotly::renderPlotly({
    ggplot(Data_frame_Boxplot_Fahrzeugsmarke(), aes(x = Motor_Typ, y = Lebensdauer_in_Days)) + geom_boxplot() + labs(x ="Motorentyp", y ="Lebensdauer des Fahrzeugs")
  })
  
  
  output$Boxplot_Fahreugtyp <- plotly::renderPlotly({
    ggplot(Data_frame_Boxplot_Fahrzeugtyp(), aes(x = Motor_Typ, y = Lebensdauer_in_Days)) + geom_boxplot() + labs(x ="Motorentyp", y ="Lebensdauer des Fahrzeugs")
  })
  
  
 output$Balkendiagramm <- plotly::renderPlotly({
    ggplot(Data_frame_Balkendiagramm(), aes(x = Motor_Typ, y = Relative_Fehlerhaufigkeit)) + geom_bar(stat = "identity") + labs(x ="Motorentyp", y ="Fehlerhaufigkeiten")
  })
