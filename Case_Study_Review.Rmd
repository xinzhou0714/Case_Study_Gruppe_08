...
# Entfernen alter Datensätze (Rohdaten)
remove(Einzelteil_T01_roh, Einzelteil_T02_roh, Einzelteil_T03_roh, Einzelteil_T04_roh, Einzelteil_T05_roh, Einzelteil_T06_roh, Einzelteil_T07_roh, Einzelteil_T08_roh, Einzelteil_T09_roh, Einzelteil_T10_roh)
remove(Fahrzeuge_OEM1Typ11_roh, Fahrzeuge_OEM1Typ12_roh, Fahrzeuge_OEM2Typ21_roh, Fahrzeuge_OEM2Typ22_roh)
remove(Komponente_K1BE1_roh, Komponente_K1BE2_roh, Komponente_K1DI1_roh, Komponente_K1DI2_roh)
remove(Zulassungen_alle_Fahrzeuge_roh)
remove(Bestandteile_Fahrzeuge_OEM1Typ11_roh, Bestandteile_Fahrzeuge_OEM1Typ12_roh, Bestandteile_Fahrzeuge_OEM2Typ21_roh, Bestandteile_Fahrzeuge_OEM2Typ22_roh)
remove(Bestandteile_Komponente_K1BE1_roh, Bestandteile_Komponente_K1BE2_roh, Bestandteile_Komponente_K1DI1_roh, Bestandteile_Komponente_K1DI2_roh)
...
```{r EchteApp, message = TRUE, warning = TRUE}
...
tags$style(HTML("#title{color: white; font-style: normal; font-family: Optima, serif; font-size: 28px;}"))), 
    
    column(width = 2, img(src='https://lh3.googleusercontent.com/2ggUfxDZyxiE59_DF-CGC42AIIo5sJT_sMIWyBqpcQ5Uu8RB8Tg9qbx6ymCUKsWzGNs', align = "left", width = 100,
         height = 100))),
...
# Anpassung des Hintergrundformats der Applikation  
  ...
  setBackgroundColor(color = c("DarkRed", "white"),
                      gradient = "linear",
                      direction = "bottom"),
...
# Outputs (Zweite Aufgabe der Visualisierung - Lebensdauer der Fahrzeuge)
         ...
         tabPanel(
           "Fahrzeugtyp", plotly::plotlyOutput("Boxplot_Fahreugtyp_11"), plotly::plotlyOutput("Boxplot_Fahreugtyp_12"), plotly::plotlyOutput("Boxplot_Fahreugtyp_21"), plotly::plotlyOutput("Boxplot_Fahreugtyp_22"), 
                #   fluidRow(
                #         column(6,
                #                 plotly::plotlyOutput("Boxplot_Fahreugtyp_11")
                #         ),
                #         column(6,
                #                plotly::plotlyOutput("Boxplot_Fahreugtyp_11")
                #         )
  ...
  #                 , 
  # fluidRow(
  #   column(6, plotly::plotlyOutput("Boxplot_Fahreugtyp_11")),
  #   column(6, plotly::plotlyOutput("Boxplot_Fahreugtyp_12"))),
  # fluidRow(
  #       column(6, "Level 2"),
  #       column(6, "Level 2")
  #     )
...
# Vorbearbeitung des Datensatzes beim Starten der App (zum schelleren Laden)

Final_Datensatz_reduziert <- Final_Datensatz %>%
  dplyr::select(c("Motor_Typ","Lebensdauer_in_Days", "Produktionsdatum", "Fahrzeug_Typ")) %>%
  dplyr::filter(!is.na(Lebensdauer_in_Days))

Final_Datensatz_reduziert11 <- Final_Datensatz_reduziert %>%
  dplyr::filter( Fahrzeug_Typ == "11") %>%
  dplyr::select(-Fahrzeug_Typ)

Final_Datensatz_reduziert12 <- Final_Datensatz_reduziert %>%
  dplyr::filter( Fahrzeug_Typ == "12") %>%
  dplyr::select(-Fahrzeug_Typ)

Final_Datensatz_reduziert21 <- Final_Datensatz_reduziert %>%
  dplyr::filter( Fahrzeug_Typ == "21") %>%
  dplyr::select(-Fahrzeug_Typ)

Final_Datensatz_reduziert22 <- Final_Datensatz_reduziert %>%
...
# Angepasster Datensatz für den Boxplot der Fahrzeugtyp als Reactive Expression (Zweite Aufgabe der Visualisierung - Lebensdauer der Fahrzeuge)
  ...
  Data_frame_Boxplot_Fahrzeugtyp_11 <- reactive({
    ...
    if (input$Fahrzeugtyp_11){
  
      Final_Datensatz_reduziert11 %>%
  ...
  Data_frame_Boxplot_Fahrzeugtyp_12 <- reactive({
     ...
     if (input$Fahrzeugtyp_12){
         
      Final_Datensatz_reduziert12 %>%
  ...
  Data_frame_Boxplot_Fahrzeugtyp_21 <- reactive({
    ...
    if (input$Fahrzeugtyp_21){
  
      Final_Datensatz_reduziert21 %>%
  ...
  Data_frame_Boxplot_Fahrzeugtyp_22 <- reactive({
     ...
     if (input$Fahrzeugtyp_22){
         
      Final_Datensatz_reduziert22 %>%
...
# Erstellen der Reactive Endpoint für den Balkendiagramm (Erste Aufgabe der Visualisierung - Fehlerhäufigkeit der Motoren)  
  ...
  output$Balkendiagramm <- plotly::renderPlotly({
    ggplot(Data_frame_Balkendiagramm(), aes(x = Motor_Typ, y = Relative_Fehlerhaufigkeit)) + geom_bar(stat = "identity", fill = "DarkRed", width = 0.5) + labs(x ="Motorentyp", y ="Fehlerhaufigkeit", title = "Relative Fehlerhäufigkeit der Benzin- und
Dieselmotoren") + theme_minimal() + theme(text = element_text(family = "Bookman"), title = element_text(color = "gray25"), plot.title = element_text(size=12, margin=margin(0,0,60,0)), plot.background = element_rect(fill = "gray95"), plot.margin = unit(c(10, 10, 10, 10), units = "mm"))
  })
...
# Erstellen der Reactive Endpoint für den Fahrzeugtyp (Zweite Aufgabe der Visualisierung - Lebensdauer der Fahrzeuge)  
  ...
  Fehlerdatum (Tage)", title = "Lebensdauer der Fahrzeuge von Typ 11") + theme_minimal() + theme(text = element_text(family = "Bookman"), title = element_text(color = "gray25"), plot.background = element_rect(fill = "gray95"), plot.title = element_text(size=12, margin=margin(0,0,30,0)), plot.margin = unit(c(10, 10, 10, 10), units = "mm"), axis.title.y = element_text(margin(t = 0, r = 60, b = 0, l = 0)), axis.title.x = element_text(margin(t = 40, r = 0, b = 0, l = 0)) )
   }
    else{ggplot()}
  })
  ...
  Fehlerdatum (Tage)", title = "Lebensdauer der Fahrzeuge von Typ 12") + theme_minimal() + theme(text = element_text(family = "Bookman"), title = element_text(color = "gray25"), plot.title = element_text(size=12, margin=margin(0,0,30,0)), plot.background = element_rect(fill = "gray95"), plot.margin = unit(c(10, 10, 10, 10), units = "mm"))
    }
     else{ggplot()}
  })
   ...
   output$Boxplot_Fahreugtyp_21 <- plotly::renderPlotly({
    ...
    ggplot(Data_frame_Boxplot_Fahrzeugtyp_21(), aes(x = Motor_Typ, y = Lebensdauer_in_Days)) + geom_boxplot(fill = "DarkRed", outlier.colour = "red", outlier.shape = 1) + labs(x ="Motorentyp", y ="Zeitintervall zwischen Zulassung und 
  Fehlerdatum (Tage)", title = "Lebensdauer der Fahrzeuge von Typ 21") + theme_minimal() + theme(text = element_text(family = "Bookman"), title = element_text(color = "gray25"), plot.title = element_text(size=12, margin=margin(0,0,30,0)), plot.background = element_rect(fill = "gray95"), plot.margin = unit(c(10, 10, 10, 10), units = "mm"))
    }
     else{ggplot()}
  })
   ...
   output$Boxplot_Fahreugtyp_22 <- plotly::renderPlotly({
    ...
    ggplot(Data_frame_Boxplot_Fahrzeugtyp_22(), aes(x = Motor_Typ, y = Lebensdauer_in_Days)) + geom_boxplot(fill = "DarkRed", outlier.colour = "white", outlier.shape = NA) + labs(x ="Motorentyp", y ="Zeitintervall zwischen Zulassung und 
  Fehlerdatum (Tage)", title = "Lebensdauer der Fahrzeuge von Typ 22") + theme_minimal() + theme(text = element_text(family = "Bookman"), title = element_text(color = "gray25"), plot.title = element_text(size=12, margin=margin(0,0,30,0)), plot.background = element_rect(fill = "gray95"), plot.margin = unit(c(10, 10, 10, 10), units = "mm"))
    }
     else{ggplot()}
  })
